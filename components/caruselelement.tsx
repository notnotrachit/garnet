...
export default function CaruselElement(props: { content: string, init_pos: number}) {
      let initial_class = " w-96 shadow-xl transition-all ease-in-out bg-gray-400 text-black text-center py-5 font-bold rounded-md";
      if (props.init_pos === 0) {
        ...
        initial_class += " -translate-y-[200%] scale-50 opacity-50";
      }
        ...
        function rotate_element() {
          ...
          setInterval(() => {
            if (pos === 0) {
              ...
              const orig_class = element.className;
              const new_class = orig_class.replace("-translate-y-full", "-translate-y-[200%]").replace("scale-75", "scale-50").replace("opacity-75", "opacity-50");
              element.className = new_class;
              pos = 2;
            }
            else if (pos === 2) {
              const orig_class = element.className;
              const new_class = orig_class.replace("-translate-y-[200%]", "translate-y-[200%]");
              element.className = new_class;
              pos = -2;
            }
            else if (pos === 3) {
            ...
            // else if (pos === -3) {
            //   const orig_class = element.className;
            //   const new_class = orig_class.replace("translate-y-[300%]", "translate-y-[200%]").replace("scale-0", "scale-50").replace("opacity-0", "opacity-50");
            //   element.className = new_class;
            //   pos = -2;
            // }
            else if (pos === -2) {
              const orig_class = element.className;
              const new_class = orig_class.replace("translate-y-[200%]", "translate-y-full").replace("scale-50", "scale-75").replace("opacity-50", "opacity-75");
              element.className = new_class;
              pos = -1;
            }
            else if (pos === -1) {
